...
function encrypt(codeVerifier) {
  return new Promise((resolve, reject) => {
    ...
    if(isIE()) {
      encrypt.oncomplete = (evt) => resolve(Base64url.encode(evt.target.result));
      encrypt.onerror = () => reject('error.unknown_error');
    } else {
      const onSuccess = (codeChallenge) => resolve(Base64url.encode(codeChallenge));
...
function createAuthorizeUrl(config, params) {
  return params.loginUrl + '?client_id=' + config.clientId + '&redirect_uri=' + params.redirectUri + '&response_type=' +
    config.responseType + '&state=' + params.state + '&scope=' + config.scope + '&federation_hint=' + config.federationHint +
    '&oauth2=authorize' + params.prompt + params.codeChallenge;
}
